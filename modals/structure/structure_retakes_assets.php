<?php
	...
	if (!isset($asset) && !is_a($asset, 'Assets')) die('Asset undefined !');

	try {
		$ACL = new ACL($_SESSION['user']);
		$dirRetakes		 = $asset->getDirRetakes($idProj, $deptID);
		$IDasset		 = $asset->getIDasset();
		$lastRetake		 = $asset->getLastRetake();
		$srclastRetake	 = FOLDER_DATA_PROJ.$lastRetake.'?_'.time();
		$validLastRetake = $asset->isValidLastRetake($idProj, $deptID);
		$oldRetakesList  = $asset->getRetakesList();
		$nbRetakes		 = count($oldRetakesList);
		$authPublished	 = $ACL->check('ASSETS_PUBLISH', 'asset:'.$IDasset);
	}
	...
	$authAddPublish = ($authPublished && ($validLastRetake || $nbRetakes == 0));
	$hideRetakeActuelle = ($nbRetakes == 0) ? 'hide' : '';

	// Formattage du nom des fichiers retakes à télécharger
	$targetName = 'SaAMpub_'.preg_replace('/ /', '-', $titleProj).'_asset_'.preg_replace('/ |_/', '-', $nameAsset).'_'.$dept;
?>

<script>
	var retakeName = '<?php echo $JSretakeName; ?>';
	var authAddRetake = <?php echo ($authAddPublish) ? 'true' : 'false'; ?>;
	...
	$(function(){
		...
		$('.oldRetakeHead').click(function(){
			if ($(this).attr('opened') != 'opened') {
				...
				$('#assetViewLeft').load('modals/structure/structure_messages_assets.php', {nameAsset:nameAsset, idProj:idProj, deptID: deptID, retakeNum:rNum, authAdd: false}, function(){
				...
				$('#assetViewLeft').load('modals/structure/structure_messages_assets.php', {nameAsset:nameAsset, idProj:idProj, deptID: deptID}, function(){
					recalc_scrolls_asset();
					if (!authAddRetake)
		...
		// Si asset pas validé dans xml ou pas encore de step
		$('#btn_addRetakeError').click(function(){
			alert('The asset must be validated in tree,\nand a step must be specified in order to add a publish.');
		});
		...
		$('#commitNewRetake').click(function(){
			...
			var ajaxReq = "action=moveTempRetake&idProj="+idProj+"&deptID="+deptID+"&nameAsset="+nameAsset+"&retakeTempName="+retakeName;
		...
		$('#validRetake').click(function(){
			...
			var ajaxReq = "action=valideRetake&idProj="+idProj+"&deptID="+deptID+"&nameAsset="+nameAsset;
			AjaxJson(ajaxReq, "depts/assets_actions", retourAjaxAssets, true);
		});
		...
		$('#retakesList').on('click', '#commitModRetake', function(){
			var ajaxReq = "action=moveTempRetake&idProj="+idProj+"&deptID="+deptID+"&nameAsset="+nameAsset+"&retakeTempName="+retakeName+'&modif=true';
			AjaxJson(ajaxReq, "depts/assets_actions", retourAjaxAssets, true);
		});
	...
	<div class="fondSect3 colorSoft retakeVignActive <?php echo $hideRetakeActuelle; ?>" id="activeRetakeVignette">
		<?php $typeLastRetake = check_mime_type(INSTALL_PATH.FOLDER_DATA_PROJ.$lastRetake);
			...
			if (preg_match('/video/i', $typeLastRetake)) :
				...
				$vignetteRetakeVideo	= $pathRetake.'/thumbs/vthumb_'.$retakeName.'.gif';
				...
				<a class="fancyboxVideo" href="#videoLastRetake_<?php echo $IDasset; ?>_0">
					<img src="<?php echo $vignetteRetakeVideo; ?>" width="270" height="150" class="noMarge" id="vi"  help="video_Retake" />
				</a>
				<div class="hide" id="videoLastRetake_<?php echo $IDasset; ?>_0">
					<video src="<?php echo "$pathRetake/video_$retakeName.ogv"; ?>" width="960" heigth="540" preload="none" controls="controls" loop="loop" poster="<?php echo $vignetteRetakeVideo; ?>" />
				</div>
		<?php else: ?>
	...
	if (is_array($oldRetakesList)) :
		foreach ($oldRetakesList as $nR => $retakeName) :
			...
			<div class="fondSect3 colorSoft retakeVign">
				...
				if ($isVideo == 'true') : ?>
					<a class="fancyboxVideo" href="#videoRetake_<?php echo $IDasset.'_'.$nR; ?>">
						<img src="" width="270" height="150" class="margeTop5 oldVignette hide" />
					</a>
					<div class="hide" id="videoRetake_<?php echo $IDasset.'_'.$nR; ?>">
