...
<script>
	$(function() {
		$('#searchAssetCaseSens').button().click(function(){
			$('#searchAssetsInput').focus().keyup();
		});

		$('#searchAssetsInput').focus().keyup(function(){
			var term = $(this).val();
			var caseSens = $('#searchAssetCaseSens').is(':checked');
			var modif = (caseSens) ? "" : "i";
			if (term == '') {
				$('#nothingFoudnMsg').show();
				$('.assetItemTag').hide();
				return;
			}
			$('#nothingFoudnMsg').hide();
			var terme = term.replace(/([.?*+^$[\]\\(){}|-])/g, "\\$1");
			var re = new RegExp(terme, modif);
			$('.assetItemTag').hide().filter(function() {
				return re.test($(this).attr('filename'));
			}).show();
		});
...
<div class="pad5 margeTop5">
	<input type="text" class="inline mid ui-corner-all noBorder pad3 fondSect4" style="width: 80%;" id="searchAssetsInput" />
	<div class="inline mid pico">
		<input type="checkbox" id="searchAssetCaseSens" /><label for="searchAssetCaseSens" title="Click to make the search case-sensitive."><span class="peta">aA</span></label>
	</div>
</div>

<div id="assetsList">
	<div class="colorDiscret padV5" id="nothingFoudnMsg">&nbsp;Type your search ^</div>
	<?php foreach($assetsList as $assetName => $asset) :
		...
		<div class='<?php echo $classAsset; ?> doigt assetItemTag hide' title="<?php echo $warnAsset; ?>" filename='<?php echo $assetName; ?>' filePath='<?php echo $asset[Assets::ASSET_PATH_REL]; ?>'>
