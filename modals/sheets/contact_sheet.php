<?php
	...
	elseif (isset($tagName) && !empty($tagName) && $tagName != '') {
		$dept = false;
		$titleDept = 'Tag <span class="tagItem">'.$tagName.'</span>';
...
<body>
	...
	<div class="headPage">
		...
		<div class="headTitle">
			<?php echo strtoupper(@$titleProj).' '. $titleDept; ?>: Contact Sheet
		</div>
		<div class="headInfo">
	...
	<?php foreach ($vignettes as $posSeq => $items):
			...
			<?php foreach ($items as $item => $infos) :
				...
				elseif ($item == 'titleSeqProj' && $infos == '') {
					echo '</div><div class="CSsequence pageBreakI"><div class="seqTitle">'.$titleSeq.'</div>';
					continue;
				}
				if (in_array($infos[Shots::SHOT_ID_SHOT], $exclude_shots)) continue;?>
				<div class="contactSheetItem">
					<div class="CSitemImg">
						<img src="../../<?php echo check_shot_vignette_ext($proj_ID, $labelSeq, $infos[Shots::SHOT_LABEL], $dept); ?>" />
					</div>
